...
export default function ProductImagesPage() {
  ...
  const load = useCallback(async (p = page) => {
    setLoading(true);
    try {
      const res = await getMediaFiles({ type: "image", page: p, q: q || undefined, limit: 24, folder: "products" });
      setImages(res.data);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await load(1);
      setPage(1);
      Swal.fire({ icon: "success", title: `${files.length} product image(s) uploaded`, timer: 1500, showConfirmButton: false });
    } catch (err: unknown) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Products ke liye upload ki gayi saari images aur unke links</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          ...
          <p className="text-lg font-medium">Koi product image nahi mili</p>
          <p className="text-sm mt-1">Upar "Upload Product Images" button se naye images add karo</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {images.map(img => (
            <div key={img._id} className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 border hover:border-[#f26b31] transition-all">
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-2 gap-1">
                ...
                >
                  ...
                  {copied === img._id ? "Copied!" : "Copy URL"}
                </button>
                <button
                  title="Delete"
